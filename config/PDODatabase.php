<?php
	...
	class PdoDatabase {
		...
		private $pdo;
		/** @var null the PDO-specific DSN that may be used to establish the connection */
		private $dsn;
		 ...
		 * @param null $pdoDsn (optional) the PDO-specific DSN that may be used to establish the connection
		 * @param bool|null (optional) $preserveOldState whether the old state of the connection should be preserved
		 */
		private function __construct(PDO $pdoInstance = null, $pdoDsn = null, $preserveOldState = null) {
		 ...
		 * @return static the new instance
		 */
		public static function fromDsn($pdoDsn) {
		 ...
		 * @return static the new instance
		 */
		public static function fromDataSource($pdoDataSource) {
		...
		public function update($tableName, array $updateMappings, array $whereMappings) {
			...
			foreach ($updateMappings as $updateColumn => $updateValue) {
				// Dont allow illegal column to pass
				if(is_int($updateColumn) && !isSizedString($updateColumn)) continue;
		...
		public function exec($statement, array $bindValues = null) {
			...
			// return !isSizedArray($stmt->errorInfo()) ? true : false;
			return $affectedRows;
		}
		...
		private function ensureConnected()
		{
			...
			if (!isset($GLOBALS['db'])/* || @$GLOBALS['db'] === null || @empty($GLOBALS['db']['conn'])*/)
			{
				...
				if(!empty($this->onConnectListeners))
				{
		...
		private function selectInternal(callable $callback, $query, array $bindValues = null) {
			...
			try {
				...
				$msg = $e->getMessage();
				if(strpos($msg, "mod_") !== false) throw new ModulePDOException;
