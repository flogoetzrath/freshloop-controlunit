<?php
	...
	require_once(LIBRARY_PATH."/ResponseHeader.class.php");
	require_once(LIBRARY_PATH."/Cookie.class.php");
	...
	final class Auth extends UserManager
	{
		...
		/**
		 * @param string      $ipAddress                 the IP address that should be used instead of the default
		 *                                               setting (if any), e.g. when behind a proxy
		 * @param string|null $dbTablePrefix             (optional) the prefix for the names of all database tables
		 *                                               used by this component
		 * @param bool|null   $throttling                (optional) whether throttling should be enabled (e.g. in
		 *                                               production) or disabled (e.g. during development)
		 * @param int|null    $sessionResyncInterval     (optional) the interval in seconds after which to
		 *                                               resynchronize the session data with its authoritative source
		 *                                               in the database
		 *
		 * @throws AuthError
		 * @throws DatabaseError
		 */
		public function __construct($ipAddress = null, $dbTablePrefix = null, $throttling = null, $sessionResyncInterval = null)
		{

			parent::__construct($dbTablePrefix);
		 ...
		 * @throws AuthError if an internal problem occurred (do *not* catch)*@throws DuplicateUsernameException
		...
		public function register($email, $password, $username = null, callable $callback = null)
		{
			...
			global $email_exists, $duplicate_username, $invalid_email, $invalid_password;
			...
			$err) {
				...
				if($err instanceof EmailAlreadyExistsException) $this->addFrontendError($email_exists[$GLOBALS['lang']]);
				if($err instanceof DuplicateUsernameException) $this->addFrontendError($duplicate_username[$GLOBALS['lang']]);
				if($err instanceof InvalidEmailException) $this->addFrontendError($invalid_email[$GLOBALS['lang']]);
				if($err instanceof InvalidPasswordException) $this->addFrontendError($invalid_password[$GLOBALS['lang']]);
		...
		public function registerWithUniqueUsername($email, $password, $username = null, callable $callback = null)
		{
			...
			global $duplicate_username, $email_exists, $invalid_email, $invalid_password;
		...
		} // public function reigsterWithUniqueUsername()
		 ...
		 * @param int|null      $rememberDuration (optional) the duration in seconds to keep the user logged in
		 *                                        ("remember me"), e.g. `60 * 60 * 24 * 365.25` for one year
		 * @param callable|null $onBeforeSuccess  (optional) a function that receives the user's ID as its single
		 *                                        parameter and is executed before successful authentication; must
		 *                                        return `true` to proceed or `false` to cancel
		 *
		 * @throws AmbiguousUsernameException
		 * @throws AttemptCancelledException if the attempt has been cancelled by the supplied callback that is
		 *                                   executed before success
		 * @throws AuthError if an internal problem occurred (do *not* catch)
		 * @throws TooManyRequestsException if the number of allowed attempts/requests has been exceeded
		...
		public function login($email, $password, $rememberDuration = null, callable $onBeforeSuccess = null)
		{
			...
			global $email_not_verified, $invalid_email, $unknown_username, $invalid_password, $too_many_requests;
			...
			$err) {
				...
				if($err instanceof EmailNotVerifiedException) $this->addFrontendError($email_not_verified[$GLOBALS['lang']]);
				if($err instanceof InvalidEmailException) $this->addFrontendError($invalid_email[$GLOBALS['lang']]);
				if($err instanceof UnknownUsernameException) $this->addFrontendError($unknown_username[$GLOBALS['lang']]);
				if($err instanceof InvalidPasswordException) $this->addFrontendError($invalid_password[$GLOBALS['lang']]);

				if($err instanceof TooManyRequestsException) $this->addBackendError($too_many_requests[$GLOBALS['lang']]);
		...
		public function loginWithUsername($username, $password, $rememberDuration = null, callable $onBeforeSuccess = null)
		{
			...
			global $email_not_verified, $invalid_email, $unknown_username, $ambiguous_username, $invalid_password, $too_many_requests, $email_or_username_required;
			...
			$err) {
				...
				if($err instanceof UnknownUsernameException) $this->addFrontendError($unknown_username[$GLOBALS['lang']]);
				if($err instanceof AmbiguousUsernameException) $this->addFrontendError($ambiguous_username[$GLOBALS['lang']]);
				if($err instanceof InvalidPasswordException) $this->addFrontendError($invalid_password[$GLOBALS['lang']]);

				if($err instanceof EmailOrUsernameRequiredError) $this->addFrontendError($email_or_username_required[$GLOBALS['lang']]);
				if($err instanceof EmailNotVerifiedException) $this->addBackendError($email_not_verified[$GLOBALS['lang']]);
				if($err instanceof InvalidEmailException) $this->addBackendError($invalid_email[$GLOBALS['lang']]);
				if($err instanceof TooManyRequestsException) $this->addBackendError($too_many_requests[$GLOBALS['lang']]);
		...
		} // public function reconfirmPassword()
		...
		private function createRememberDirective($userId, $duration)
		{
			...
			$selector = createRandomString(24);
			$token = createRandomString(32);
		...
		} // private function createRememberDirective()
		...
		private function setRememberCookie($selector, $token, $expires)
		{
			...
			if ($result === false) throw new HeadersAlreadySentError();
		 ...
		 * @param string   $selector         the selector from the selector/token pair
		 * @param string   $token            the token from the selector/token pair
		 * @param int|null $rememberDuration (optional) the duration in seconds to keep the user logged in ("remember
		 *                                   me"), e.g. `60 * 60 * 24 * 365.25` for one year
		 *
		 * @return string[] an array with the old email address (if any) at index zero and the new email address (which
		 *                  has just been verified) at index one
		 * @throws InvalidSelectorTokenPairException if either the selector or the token was not correct
		 * @throws TokenExpiredException if the token has already expired
		 * @throws UserAlreadyExistsException if an attempt has been made to change the email address to a (now)
		 *                                    occupied address
		 * @throws TooManyRequestsException if the number of allowed attempts/requests has been exceeded
		 * @throws AuthError if an internal problem occurred (do *not* catch)*@throws InvalidEmailException
		...
		} // public function confirmEmailAndSignIn()
		 ...
		 *
		 * @throws NotLoggedInException if the user is not currently signed in
		 * @throws InvalidPasswordException if either the old password has been wrong or the desired new one has been
		 *                                  invalid
		 * @throws TooManyRequestsException if the number of allowed attempts/requests has been exceeded
		 * @throws AuthError if an internal problem occurred (do *not* catch)*@throws UnknownIdException
		 * @throws UnknownIdException
		 */
		...
		} // public function changePasswordWithoutOldPassword()
		 ...
		 *
		 * @throws AuthError AuthError
		 * @throws ConfirmationRequestNotFound if no previous request has been found that could be re-sent
		 * @throws TooManyRequestsException if the number of allowed attempts/requests has been exceeded*@throws
		 */
		...
		} // public function resendConfirmationForUserId()
		...
		} // private function resendConfirmationForColumnValue()
		...
		public function forgotPassword($email, callable $callback, $requestExpiresAfter = null, $maxOpenRequests = null)
		{
			...
			else $requestExpiresAfter = (int) $requestExpiresAfter;
			...
			else $maxOpenRequests = (int) $maxOpenRequests;
			...
			if ((int) $userData['verified'] !== 1) throw new EmailNotVerifiedException();

			// do not allow a password reset if the user has explicitly disabled this feature
			if ((int) $userData['resettable'] !== 1) throw new ResetDisabledException();
			...
			else throw new TooManyRequestsException('', $requestExpiresAfter);
		 ...
		 * @throws DatabaseError
		 * @throws EmailNotVerifiedException if the email address has not been verified yet via confirmation email
		 * @throws EmailOrUsernameRequiredError
		 * @throws InvalidEmailException if the email address was invalid or could not be found
		...
		private function authenticateUserInternal($password, $email = null, $username = null, $rememberDuration = null, callable $onBeforeSuccess = null)
		{
			...
			else throw new EmailOrUsernameRequiredError();
			...
			if (password_verify($password, $userData['password']))
			{
				...
				if ((int) $userData['verified'] === 1)
				{
					...
					if (!isset($onBeforeSuccess) || (is_callable($onBeforeSuccess) && $onBeforeSuccess($userData['id']) === true))
					{
						...
						if ($rememberDuration === true) $rememberDuration = 60 * 60 * 24 * 28;
						else if ($rememberDuration === false) $rememberDuration = null;
						if ($rememberDuration !== null)
							$this->createRememberDirective($userData['id'], $rememberDuration);
		...
		private function getUserDataByEmailAddress($email, array $requestedColumns)
		{
			...
			if(is_object($userData)) $userData = json_decode(json_encode($userData), true);

			if (!empty($userData)) return $userData;
			else throw new InvalidEmailException();

		} // private function getUserDataByEmailAdress()
		...
		} // private function getOpenPasswordResetRequest()
		...
		private function createPasswordResetRequest($userId, $expiresAfter, callable $callback)
		{
			...
			$selector = createRandomString(20);
		...
		public function resetPassword($selector, $token, $newPassword)
		{
			...
			if (!empty($resetData))
			{
				...
				else throw new ResetDisabledException();
			}
			else throw new InvalidSelectorTokenPairException();
		 ...
		 * @param string $token    the token from the selector/token pair
		 *
		 * @throws AuthError if an internal problem occurred (do *not* catch)*@throws AuthException
		...
		} // public function canResetPasswordOrThrow()
		...
		} // public function setPasswordResetEnabled()
		...
		public function isNormal()
		{
			...
			return $this->getStatus() === static::$status['NORMAL'];
		...
		public function isArchived()
		{
			...
			return $this->getStatus() === static::$status['ARCHIVED'];
		...
		public function isBanned()
		{
			...
			return $this->getStatus() === static::$status['BANNED'];
		...
		public function isLocked()
		{
			...
			return $this->getStatus() === static::$status['LOCKED'];
		...
		public function isPendingReview()
		{
			...
			return $this->getStatus() === static::$status['PENDING_REVIEW'];

		} // public funciton isPendingReivew()
		...
		public function isSuspended()
		{
			...
			return $this->getStatus() === static::$status['SUSPENDED'];
		...
		public function hasRole(Int $role)
		{
			...
			if (empty($role) || !is_numeric($role)) return false;
		...
		public function throttle(array $criteria, $supply, $interval, $burstiness = null, $simulated = null, $cost = null)
		{
			...
			if (is_object($bucket)) $bucket = json_decode(json_encode($bucket), true);
			...
			if ($accepted) return $bucket['tokens'];
			else
			{
		...
		public function admin()
		{
			...
			return new Administration($this->dbTablePrefix);
		...
		public static function createCookieName($descriptor, $seed = null)
		{
			...
			$token = encodeUrlSafeWithoutPadding(
				md5(
					__NAMESPACE__ . "\n" . $seed,
					true
				)
		...
		} // public static function createRememberCookieName()
